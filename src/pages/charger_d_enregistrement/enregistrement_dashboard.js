...
const EnregistrementDashboard = () => {
  ...
  const fetchEnregistrementStats = async () => {
    try {
      ...
      console.log(filteredFicheEssai);
        ...
        },
        {
          ...
          trend: calculateTrend(
            ...
            filteredFicheEssai.filter(
              (f) => new Date(f.osDate) >= previousPeriodDate
            ).length
          ),
        },
        {
  ...
  useEffect(() => {
    ...
    if (user.roles.includes("chargé_denregistrement")) {
      fetchEnregistrementStats();
    } else {
      setError(
        "Accès non autorisé. Cette page est réservée au chargé d'enregistrement."
      );
      navigate("/login");
    }
    setLoading(false);
